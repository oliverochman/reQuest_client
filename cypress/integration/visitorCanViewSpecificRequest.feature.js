  ...
  describe('as a visitor', () => {
    it('a button should not exist when visitor is browsing', () => {
      ...
      cy.get("#selected-request").within(() => {
        ...
        cy.get("#selected-message").should("contain", "Log in or sign up to offer your help")
      })
    })
  })
  ...
  describe('as a user', () => {
    ...
    it('a request can have contact button disabled if the user has already offered', () => {
      ...
      cy.get("#selected-request").within(() => {
        ...
        cy.get("button").should("be.disabled")
        cy.get("#selected-message").should("contain", "You have already offered to help with this request")
      });
    });
    ...
    it('a request can have contact button disabled if the user made the request', () => {
      ...
      cy.get("#selected-request").within(() => {
        ...
        cy.get("#selected-description").should("contain", "Need a tongue with French")
        cy.get("#selected-requester").should("contain", "user@mail.com")
        cy.get("button").should("be.disabled")
        cy.get("#selected-message").should("contain", "You cannot make an offer on your own request")
      });
    });
  })
